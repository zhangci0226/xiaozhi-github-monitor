...
- 自动附带项目现状：最新版本、发布时间、Release 链接、当前主要功能、升级/兼容注意。
- 可选调用 DeepSeek，把技术变更整理成“新增了什么、改变了什么、要注意什么”。
- 定时运行没有变化时默认不推送；手动测试 `force_send=true` 时会推送项目现状。
...
2. 项目现状：展示最新版本、发布时间、Release 链接、当前主要功能和升级注意。
3. 技术明细：保留 commit、PR、issue、关键文件链接，方便你想深入时点开看。
